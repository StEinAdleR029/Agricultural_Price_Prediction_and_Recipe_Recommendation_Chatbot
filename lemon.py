...
lemon_loss = 0
lemon_grad = 0
lemon_estimate_price = 0

def main():

    global lemon_loss
    global lemon_grad
    global lemon_estimate_price
    
    df = pd.read_csv('lemon11.csv', encoding='euc-kr')
    data = np.array(df)
    date = '2022-10-25'    
    ...
    if k<=div-1:
      ...
      lemon_loss += round(loss,2)
      lemon_grad += beta1
      lemon_estimate_price += round(beta1 * X[k] + beta0, 2)
      ...
      lemon_loss += round(loss,2)
      lemon_grad += beta1
      lemon_estimate_price += round(beta1 * X[k-div] + beta0, 2)
...
def get_lemon_estimate_price():
    global lemon_estimate_price
    main()
    c = lemon_estimate_price
    return c