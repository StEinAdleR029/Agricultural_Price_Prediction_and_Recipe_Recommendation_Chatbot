...
def main():
    ...
    df = pd.read_csv('potato11.csv', encoding='euc-kr')
    data = np.array(df)
    ...
    if k<=div1-1:
      ...
      potato_loss += round(loss,2)
      potato_estimate_price += round(beta1 * X[k-div1] + beta0, 2)
...
def get_potato_estimate_price():
    global potato_estimate_price
    main()
    c = potato_estimate_price
    return c
