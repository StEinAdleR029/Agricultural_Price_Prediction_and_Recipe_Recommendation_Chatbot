...
def main():
    ...
    df = pd.read_csv('paprika11.csv', encoding='euc-kr')
    data = np.array(df)
    date = '2022-08-14'
    ...
    if k<=div1-1:
      ...
      paprika_loss += round(loss,2)
      paprika_grad += beta1
      paprika_estimate_price += round(beta1 * X[k] + beta0, 2)
      plt.scatter(X,Y)
      ...
      paprika_loss += round(loss,2)
      paprika_grad += beta1
      paprika_estimate_price += round(beta1 * X[k-div1] + beta0, 2)
      plt.scatter(X,Y)
      ...
      paprika_loss += round(loss,2)
      paprika_grad += beta1
      paprika_estimate_price += round(beta1 * X[k-div2] + beta0, 2)
      plt.scatter(X,Y)
      plt.plot(X,price_pred, color='red')
      plt.grid()
    
def get_paprika_loss():
    global paprika_loss
    main()
    a = paprika_loss
    return a

def get_paprika_grad():
    global paprika_grad
    main()
    b = paprika_grad
    return b

def get_paprika_estimate_price():
    global paprika_estimate_price
    main()
    c = paprika_estimate_price
    return c