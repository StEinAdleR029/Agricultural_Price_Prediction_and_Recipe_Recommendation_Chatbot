...
banana_loss = 0
banana_grad = 0
banana_estimate_price = 0

def main():

    global banana_loss
    global banana_grad
    global banana_estimate_price
    
    df = pd.read_csv('banana11.csv', encoding='euc-kr')
    data = np.array(df)
    date = '2022-10-25'    
    ...
    if k<=div-1:
      ...
      banana_loss += round(loss,2)
      banana_grad += beta1
      banana_estimate_price += round(beta1 * X[k] + beta0, 2)
      ...
      banana_loss += round(loss,2)
      banana_grad += beta1
      banana_estimate_price += round(beta1 * X[k-div] + beta0, 2)
...
def get_banana_estimate_price():
    global banana_estimate_price
    main()
    c = banana_estimate_price
    return c